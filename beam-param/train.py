...
def createmodel(opt, output_size, normalizer):
    model = Sequential()
...
def main():
    ...
    # normalizery = Normalization(axis=None)
    # normalizery.adapt(np.array(trainy))
    # layernormy = normalizery(np.array(trainy))

    # bnormalizerx = BatchNormalization(input_shape=(101,4))
    
    ###########################################################################
    # Create and Train Model
    ###########################################################################
    output_size = 1
    model = createmodel(opt, output_size, normalizerx)
    ...
    #     verbose=1,
    #     callbacks=callbacks_list,
    #     validation_data=(normalizerx(np.array(validx)), normalizery(np.array(validy))),
    #     validation_freq=1,
    #     workers=12,
    #     use_multiprocessing=True
    # )
